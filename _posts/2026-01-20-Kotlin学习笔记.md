...
初学 Kotlin 可以按下面的优先级学习：

1. **基础语法**：`val`、`var`、函数、字符串模板、`if`、`when`、循环。
2. **空安全**：`?`、`?.`、`?:`、`let`、`requireNotNull`，以及如何减少核心逻辑中的可空类型。
3. **类与对象**：主构造器、属性、数据类、接口、继承、伴生对象。
4. **集合与 Lambda**：`map`、`filter`、`fold`、`groupBy`、高阶函数、作用域函数。
5. **类型建模**：枚举、密封类、密封接口、泛型、型变。
6. **协程**：`suspend`、`coroutineScope`、`launch`、`async`、`withContext`、Flow。
7. **工程实践**：Gradle、测试、Java 互操作、Android/KMP/服务端框架。

一个有效的学习策略是：先用 Kotlin 写小程序和单元测试，熟悉语法；再读项目里的真实 Kotlin 代码，理解 API 设计方式；最后学习协程、Flow 和框架集成。
...
### 2.10 Kotlin Multiplatform 项目结构
...
- IDE Kotlin 插件或 Android Studio 版本不匹配。
- Gradle 项目没有重新同步。
- IDE 使用的 JDK 与 Gradle toolchain 不一致。
- KSP、Compose、AGP 等插件版本不兼容。

处理顺序：

1. 用命令行确认 `./gradlew build` 是否成功。
2. 检查 IDE 使用的 Gradle JDK。
3. 重新 Sync Gradle。
4. 检查 Kotlin、AGP、KSP、Compose Compiler 版本兼容。
5. 必要时清理 IDE 缓存。
...
- IDE 使用了本地特殊配置。
- 环境变量、JDK、Android SDK 路径在终端中不同。
- CI 或终端没有接受 Android SDK license。
- 依赖仓库访问受网络或代理影响。

处理方式：

- 优先让命令行构建成为事实标准。
- 通过 Gradle Wrapper 执行任务。
- 把必要配置写入项目文档或 CI 配置，不依赖个人 IDE。
...
- 源文件统一使用 UTF-8。
- Gradle JVM 参数中设置 `-Dfile.encoding=UTF-8`。
- Windows 终端显示乱码时，先确认文件编码和终端编码，不要直接改文件内容。
- 仓库路径尽量避免过长路径和特殊符号，尤其是老工具链或 Android 构建链路。
