...
```mermaid
architecture-beta
  group app(cloud)[应用系统]

  service web(server)[Web 前端] in app
  service api(server)[API 服务] in app
  service db(database)[数据库] in app
  service cache(database)[缓存] in app
...
- Official: Mermaid Gitgraph Diagram Syntax: https://mermaid.js.org/syntax/gitgraph.html
