...
这一章讲的是 **04. 工具调用、Function Calling 与 MCP**。学习 AI Agent 时最容易犯的错误，是把所有新名词都看成“更高级的聊天机器人功能”。更稳妥的学法是：先看它解决什么工程问题，再看它把模型、工具、上下文、状态、评测和安全中的哪一环变得更清楚。

### 一句话先懂

工具调用解决“模型怎么动手”，MCP 解决“不同 AI 应用如何用统一协议连接工具、资源和提示模板”。

### 通俗类比

Function Calling 像模型和函数之间的点对点插座；MCP 更像统一规格的插排和接口标准，让不同应用更容易接入文件、数据库、浏览器、IDE 或业务系统。
...
设计一个只读 MCP 工具，例如查询文档或列出项目文件，写出工具名、输入 schema、权限、超时、错误码和 mock 返回。
...
能区分 Function Calling、Tool Gateway 和 MCP；能解释 Host、Client、Server、Tools、Resources、Prompts；能为写操作设计审批、幂等和审计。
