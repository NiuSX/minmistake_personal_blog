...
这一章讲的是 **06. 规划、工作流与执行**。学习 AI Agent 时最容易犯的错误，是把所有新名词都看成“更高级的聊天机器人功能”。更稳妥的学法是：先看它解决什么工程问题，再看它把模型、工具、上下文、状态、评测和安全中的哪一环变得更清楚。

### 一句话先懂

规划不是让模型无限自由发挥，而是把复杂任务拆成可检查、可停止、可回滚的步骤。

### 通俗类比

Workflow 像固定路线公交车，Agent 规划像司机根据路况选择路线。生产系统通常不是全靠司机自由开，而是在固定站点、限速、导航和调度规则内允许调整。
...
把一个“生成周报并发送审批”的任务拆成确定性工作流和模型判断步骤，标出重试、人工审批和停止条件。
...
能区分固定 Workflow、Planner、ReAct、Reflection 和 Human-in-the-loop；能为长任务设计状态机；能限制最大步数、预算和副作用。
