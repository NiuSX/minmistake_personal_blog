...
这一章讲的是 **07. 多智能体系统**。学习 AI Agent 时最容易犯的错误，是把所有新名词都看成“更高级的聊天机器人功能”。更稳妥的学法是：先看它解决什么工程问题，再看它把模型、工具、上下文、状态、评测和安全中的哪一环变得更清楚。

### 一句话先懂

多 Agent 不是更高级的默认选项，而是在分工、审查、竞争或并行确有收益时才值得引入的复杂结构。

### 通俗类比

多 Agent 像组建项目小组。一个人能完成的任务硬拆成多人，沟通成本会变高；但研究、实现、审查、测试职责明确时，小组协作会更稳。
...
设计一个“研究员-写作者-审查员”的三角色流程，规定每个 Agent 的输入输出、共享状态和冲突处理规则。
...
能说明多 Agent 的适用场景和风险；能设计角色边界、通信协议和终止条件；能判断什么时候单 Agent 或 Workflow 更合适。
