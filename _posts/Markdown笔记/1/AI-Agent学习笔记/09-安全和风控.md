...
这一章讲的是 **09. 安全与风控**。学习 AI Agent 时最容易犯的错误，是把所有新名词都看成“更高级的聊天机器人功能”。更稳妥的学法是：先看它解决什么工程问题，再看它把模型、工具、上下文、状态、评测和安全中的哪一环变得更清楚。

### 一句话先懂

Agent 安全的重点是限制模型能接触什么、能调用什么、能影响什么，而不是相信模型会自觉守规矩。

### 通俗类比

把 Agent 安全想成给助理发门禁卡。不能因为助理会写计划，就给它财务室、服务器和客户系统的全部权限；不同房间、不同动作、不同金额都应有不同审批。
...
为一个会发送邮件或修改数据的 Agent 设计权限矩阵、审批流程、提示注入防护和审计日志字段。
...
能识别 Prompt Injection、数据泄露、越权工具调用、供应链和误操作风险；能设计只读优先、最小权限、人工审批和回滚策略。
