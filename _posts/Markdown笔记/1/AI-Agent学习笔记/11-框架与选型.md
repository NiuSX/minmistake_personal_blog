...
这一章讲的是 **11. 框架与选型**。学习 AI Agent 时最容易犯的错误，是把所有新名词都看成“更高级的聊天机器人功能”。更稳妥的学法是：先看它解决什么工程问题，再看它把模型、工具、上下文、状态、评测和安全中的哪一环变得更清楚。

### 一句话先懂

框架选型不是比谁最火，而是看你的任务需要图执行、RAG、工具协议、多 Agent、观测、部署还是企业集成。

### 通俗类比

选框架像选交通工具：步行、公交、货车和高铁都能到达，但适合的距离、载重、成本和路线完全不同。框架越强不代表越适合小任务。
...
为同一个需求分别用“裸 API + 工具函数”“LangGraph”“LlamaIndex”“CrewAI/AutoGen”写选型理由和放弃理由。
...
能按任务复杂度、团队能力、运行时控制、可观测性、生态和锁定风险做选型；能说明何时不需要 Agent 框架。
