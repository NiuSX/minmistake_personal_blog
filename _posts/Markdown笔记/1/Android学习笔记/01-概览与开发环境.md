...
这一章讲的是 **01. Android 概览与开发环境**。Android 学习最怕碎片化：今天学一个控件，明天抄一段协程，后天改一个 Gradle 配置，但不知道它们怎样组成一款稳定应用。更好的读法是把每章都放进同一条主线：生命周期、状态、数据流、线程、权限、性能、测试和发布。

### 一句话先懂

Android 开发的第一课不是写页面，而是理解平台约束：设备碎片、生命周期、权限、线程、构建和发布都会影响代码写法。

### 通俗类比

搭 Android 环境像搭工作台：Android Studio 是工具台，SDK 是零件库，模拟器和真机是测试场，Gradle 是装配线。工作台不稳，后面所有代码都会受影响。
...
创建一个最小项目，记录 Android Studio、AGP、Kotlin、compileSdk、minSdk、targetSdk 和运行设备信息，并在模拟器和真机各跑一次。

实践时要保留失败记录。Android 的很多能力只有在异常场景下才真正显形，例如旋转屏幕后状态丢失、后台恢复后重复请求、release 包混淆后崩溃、权限拒绝后流程卡死。把这些失败样本记录下来，比只保存成功代码更有价值。

### 读完本章应该能产出

能解释 Android 项目运行需要哪些工具链；能区分 compileSdk、minSdk、targetSdk；能定位常见环境、同步和运行失败。
...
### 精讲扩展 1：概览与开发环境 的生命周期、状态管理 与工程化理解
...
### 精讲扩展 2：概览与开发环境 的状态管理、单向数据流 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 3：概览与开发环境 的单向数据流、协程 Flow 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 4：概览与开发环境 的协程 Flow、架构分层 与工程化理解
