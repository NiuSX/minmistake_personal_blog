...
这一章讲的是 **02. 项目结构、Gradle、Manifest 与资源**。Android 学习最怕碎片化：今天学一个控件，明天抄一段协程，后天改一个 Gradle 配置，但不知道它们怎样组成一款稳定应用。更好的读法是把每章都放进同一条主线：生命周期、状态、数据流、线程、权限、性能、测试和发布。

### 一句话先懂

项目结构决定代码放在哪里、资源如何打包、权限如何声明、不同构建版本如何生成，是后续架构和发布的基础。

### 通俗类比

Android 项目像一座仓库：Manifest 是门牌和权限登记，res 是货架标签，src 是生产区，Gradle 是调度系统，build variants 是不同出货规格。
...
给一个新项目增加 debug/release 配置、一个资源字符串、一个权限声明和一个依赖，观察生成包和 Manifest 合并结果。

实践时要保留失败记录。Android 的很多能力只有在异常场景下才真正显形，例如旋转屏幕后状态丢失、后台恢复后重复请求、release 包混淆后崩溃、权限拒绝后流程卡死。把这些失败样本记录下来，比只保存成功代码更有价值。

### 读完本章应该能产出

能说明 app module、Manifest、res、Gradle、Version Catalog、build types 和 flavors 的作用；能排查依赖冲突和 Manifest 合并问题。
...
### 精讲扩展 2：项目结构 的状态管理、单向数据流 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 3：项目结构 的单向数据流、协程 Flow 与工程化理解
