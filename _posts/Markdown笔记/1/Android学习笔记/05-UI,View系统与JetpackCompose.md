...
这一章讲的是 **05. UI：View 系统与 Jetpack Compose**。Android 学习最怕碎片化：今天学一个控件，明天抄一段协程，后天改一个 Gradle 配置，但不知道它们怎样组成一款稳定应用。更好的读法是把每章都放进同一条主线：生命周期、状态、数据流、线程、权限、性能、测试和发布。

### 一句话先懂

UI 学习的核心不是会摆控件，而是理解界面如何由状态驱动、事件如何回流、可访问性和适配如何保证。

### 通俗类比

传统 View 像手工摆放家具，Compose 像根据房屋状态自动重画布局图。两者都要关注尺寸、状态、交互和用户反馈。
...
做一个同样的计数器或表单，用 XML/View 和 Compose 各实现一次，比较状态存放、事件处理和测试方式。

实践时要保留失败记录。Android 的很多能力只有在异常场景下才真正显形，例如旋转屏幕后状态丢失、后台恢复后重复请求、release 包混淆后崩溃、权限拒绝后流程卡死。把这些失败样本记录下来，比只保存成功代码更有价值。

### 读完本章应该能产出

能说明 View 系统和 Compose 的差异；能写基础 Composable、Modifier 和列表；能理解状态驱动 UI，而不是在 UI 中直接做业务。
...
## 万字精讲扩展：UI,View系统与JetpackCompose

> 本节为按参考笔记篇幅补充的系统化扩展内容，目标是把原有笔记从“知识点记录”扩展为“概念、原理、流程、工程实践、常见误区和复盘清单”完整学习材料。

### 精讲扩展 1：UI,View系统与JetpackCompose 的生命周期、状态管理 与工程化理解
...
### 精讲扩展 2：UI,View系统与JetpackCompose 的状态管理、单向数据流 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 3：UI,View系统与JetpackCompose 的单向数据流、协程 Flow 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 4：UI,View系统与JetpackCompose 的协程 Flow、架构分层 与工程化理解
