...
版本说明：截至 2026-06-07，Gradle 官方 Releases 页面显示 Gradle 9.5.1 是当前最新稳定版本，发布日期为 2026-05-12；官方 Release Notes 推荐从 9.5.0 升级到 9.5.1。Gradle 9.5.1 运行 Gradle 本身需要 JVM 17 到 JVM 26；Gradle Wrapper、Gradle Client、Tooling API Client 和 TestKit Client 可在 JVM 8 上运行。实际项目升级时，应同时检查 Gradle、JDK、Kotlin Gradle Plugin、Android Gradle Plugin、Spring Boot Plugin 等插件的兼容矩阵。
