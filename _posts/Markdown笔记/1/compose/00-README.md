...
这一章讲的是 **Jetpack Compose 学习笔记总览**。学习 Compose 时不要把它当成“用 Kotlin 写 XML”，而要把它理解成一套声明式 UI 系统：状态变化驱动重组，Composable 描述界面，事件向上流动，副作用被 Effect API 和生命周期约束。

### 一句话先懂

这份总览要建立 Compose 学习地图：先理解声明式 UI 和重组，再掌握状态、副作用、布局、导航、性能和测试。

### 通俗类比

Compose 像一块会根据状态自动重画的电子白板：你声明“现在应该显示什么”，框架负责更新画面；但发请求、导航、写数据库这些白板外动作必须被单独管理。
