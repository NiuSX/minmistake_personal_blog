...
这一章讲的是 **06. 架构、导航与单向数据流**。学习 Compose 时不要把它当成“用 Kotlin 写 XML”，而要把它理解成一套声明式 UI 系统：状态变化驱动重组，Composable 描述界面，事件向上流动，副作用被 Effect API 和生命周期约束。

### 一句话先懂

Compose 架构要让 UI 只描述状态和发出事件，ViewModel 处理事件并暴露 UiState，Navigation 管页面关系而不是传复杂对象。

### 通俗类比

单向数据流像水循环：状态从 ViewModel 流向 UI，用户事件从 UI 回到 ViewModel；导航像路牌，只传必要地址，不搬整车货物。
...
实现 Feed 列表到详情页：列表状态在 ViewModel，点击事件触发导航，详情页通过 id 加载数据。

实践时要保留错误版本。Compose 很多坑不会直接编译失败，而是表现为重复请求、状态丢失、列表错位、测试找不到节点、重组过多或 TalkBack 读不清。把错误写法、现象、定位工具和修复方式记录下来，比只保存正确代码更有价值。

### 读完本章应该能产出

能设计 UDF、UiState、UiEvent、Navigation Compose、状态保存和模块边界；能避免双向状态和大对象导航参数。
...
### 精讲扩展 1：架构、导航与单向数据流 的生命周期、状态管理 与工程化理解
...
### 精讲扩展 2：架构、导航与单向数据流 的状态管理、单向数据流 与工程化理解
...
复习时可以使用四个问题：它解决什么问题；它依赖什么条件；它失败时有什么表现；它如何被验证。只要这四个问题能回答清楚，说明对 $topic 的理解已经从表层进入工程层。如果回答不清楚，就回到对应章节补充例子、边界和排错方法。
### 精讲扩展 3：架构、导航与单向数据流 的单向数据流、协程 Flow 与工程化理解
