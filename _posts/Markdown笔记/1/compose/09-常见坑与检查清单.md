...
这一章讲的是 **09. 常见坑与检查清单**。学习 Compose 时不要把它当成“用 Kotlin 写 XML”，而要把它理解成一套声明式 UI 系统：状态变化驱动重组，Composable 描述界面，事件向上流动，副作用被 Effect API 和生命周期约束。

### 一句话先懂

Compose 常见坑大多不是 API 不会用，而是状态来源不清、副作用 key 错、列表 key 不稳定、生命周期收集错误或版本不匹配。

### 通俗类比

检查清单像飞行前检查：不是因为飞行员不会开飞机，而是复杂系统需要固定流程防止遗漏。
...
拿一个已有 Compose 页面按清单检查：状态源、副作用、Flow 收集、Lazy key、Modifier、语义、性能和版本。

实践时要保留错误版本。Compose 很多坑不会直接编译失败，而是表现为重复请求、状态丢失、列表错位、测试找不到节点、重组过多或 TalkBack 读不清。把错误写法、现象、定位工具和修复方式记录下来，比只保存正确代码更有价值。

### 读完本章应该能产出

能用清单审查 Compose 页面；能快速定位状态丢失、重复请求、列表错位、测试找不到节点和卡顿问题。
