...
1. 先读 `01-物理层/00-物理层总览.md`，理解比特如何变成电信号、光信号或无线电波。
2. 再读 `02-数据链路层/00-数据链路层总览.md`，理解同一链路内如何寻址、封装、校验和转发。
3. 学嵌入式或工控串口时，读 `02-数据链路层/串口通信协议总览.md`，先分清 TTL UART、RS-232、RS-485、UART 帧、Modbus RTU、自定义协议的层次。
4. 继续读 `03-网络层/00-网络层总览.md`，理解跨网络寻址、路由、分片和控制消息。
5. 再读 `04-传输层/00-传输层总览.md`，理解端到端通信、可靠性、拥塞控制和端口。
6. 读 `05-会话层/00-会话层总览.md`，理解连接会话、状态、认证上下文和调用会话。
7. 读 `06-表示层/00-表示层总览.md`，理解编码、序列化、压缩、加密和内容表示。
8. 最后读 `07-应用层/00-应用层总览.md`，把 HTTP、DNS、SMTP、DHCP 等常见协议串起来。
9. 如果学习工业通信，再读 `07-应用层/工业通信协议总览.md`，并结合 `01-物理层/RS-485.md`、`07-应用层/Modbus.md` 理解“电气层”和“应用层协议”的区别。

## 目录结构

| OSI 层 | 本目录 | 重点 |
|---|---|---|
| 第 1 层 物理层 | `01-物理层` | 信号、介质、编码、速率、双工、调制、RS-232/RS-485 |
| 第 2 层 数据链路层 | `02-数据链路层` | 帧、MAC、交换、ARP、VLAN、CAN、I2C/SPI、串口通信分类 |
| 第 3 层 网络层 | `03-网络层` | IP 地址、路由、TTL/Hop Limit、ICMP、IPsec |
| 第 4 层 传输层 | `04-传输层` | 端口、连接、可靠传输、拥塞控制、UDP/QUIC |
| 第 5 层 会话层 | `05-会话层` | 会话建立、保持、恢复、RPC、会话控制 |
| 第 6 层 表示层 | `06-表示层` | 数据表示、字符集、ASN.1、MIME、TLS |
| 第 7 层 应用层 | `07-应用层` | 面向业务的协议：HTTP、DNS、DHCP、SMTP、FTP、Modbus、CANopen |
...
| 逻辑分析仪/示波器 | UART/I2C/SPI/CAN 波形排查 | 检查波特率、时序、电平 |
