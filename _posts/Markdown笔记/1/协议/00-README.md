...
3. 继续读 `03-网络层/00-网络层总览.md`，理解跨网络寻址、路由、分片和控制消息。
4. 再读 `04-传输层/00-传输层总览.md`，理解端到端通信、可靠性、拥塞控制和端口。
5. 读 `05-会话层/00-会话层总览.md`，理解连接会话、状态、认证上下文和调用会话。
6. 读 `06-表示层/00-表示层总览.md`，理解编码、序列化、压缩、加密和内容表示。
7. 最后读 `07-应用层/00-应用层总览.md`，把 HTTP、DNS、SMTP、DHCP 等常见协议串起来。
8. 如果学习工业通信，再读 `07-应用层/工业通信协议总览.md`，并结合 `01-物理层/RS-485.md`、`07-应用层/Modbus.md` 理解“电气层”和“应用层协议”的区别。
...
| 第 2 层 数据链路层 | `02-数据链路层` | 帧、MAC、交换、ARP、VLAN、CAN、I2C/SPI |
