...
这一章讲的是 **MIME 多用途互联网邮件扩展学习笔记**，属于 **表示层协议**。学习协议时，不要只背“它是什么协议、默认端口是多少”，而要把它当成一份双方共同遵守的通信合同：谁先说话，说什么格式，字段怎么解释，什么时候确认，什么时候重传，出错时返回什么，版本升级时怎样保持兼容。

### 一句话先懂

表示层解决的是同一份数据在不同系统之间如何编码、压缩、加密、签名和兼容演进。

初学时先问三个问题：它处在哪一层，解决哪一段通信问题；它的最小报文长什么样；一次正常交互和一次失败交互分别是什么时序。

### 通俗类比

表示层像翻译和包装：内容可以一样，但要约定语言、格式、压缩方式、封条和验签方式，否则收件人看不懂或不敢信。
...
JSON、MIME、ASN.1/BER/DER、TLS、字符编码、序列化、版本兼容、证书和密钥协商。

### 最小实践任务

抓一次 TLS/HTTP 或解析一段 JSON/DER，逐字段说明编码、长度、类型和兼容性规则。
