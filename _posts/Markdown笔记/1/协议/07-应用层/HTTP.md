...
这一章讲的是 **HTTP 超文本传输协议学习笔记**，属于 **应用层协议**。学习协议时，不要只背“它是什么协议、默认端口是多少”，而要把它当成一份双方共同遵守的通信合同：谁先说话，说什么格式，字段怎么解释，什么时候确认，什么时候重传，出错时返回什么，版本升级时怎样保持兼容。

### 一句话先懂

HTTP 的核心是应用语义：方法、目标资源、状态码、头字段和缓存规则共同决定一次请求到底表达什么。
...
用 curl -v 发起 GET/POST/缓存请求，对照 RFC 9110/9111 解释请求头、响应头和状态码。
