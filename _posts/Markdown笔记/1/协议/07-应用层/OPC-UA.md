...
这一章讲的是 **OPC UA 工业通信协议学习笔记**，属于 **应用层协议**。学习协议时，不要只背“它是什么协议、默认端口是多少”，而要把它当成一份双方共同遵守的通信合同：谁先说话，说什么格式，字段怎么解释，什么时候确认，什么时候重传，出错时返回什么，版本升级时怎样保持兼容。

### 一句话先懂

OPC UA 不只是通信协议，还定义了信息模型、安全、服务和订阅机制，用来让工业数据可发现、可描述、可交互。
...
连接一个 OPC UA demo server，浏览节点、读取变量、订阅变化，并记录安全策略。
