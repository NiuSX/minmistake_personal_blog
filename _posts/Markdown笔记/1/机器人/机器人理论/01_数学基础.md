...
这一章讲的是 **01. 机器人学数学基础**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

机器人数学不是为了解题，而是为了准确描述位置、姿态、速度、不确定性和优化目标。
...
用二维位姿和三维旋转各做一个小例子，写出坐标变换、误差、协方差或优化目标的含义。
...
- 能把矩阵、向量、旋转、概率和优化概念翻译成机器人里的位置、方向、误差和控制目标。
