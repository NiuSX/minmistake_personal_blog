...
这一章讲的是 **02. 刚体运动、坐标变换与 TF**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

坐标变换是机器人系统的共同语言；frame 错了，后面的感知、定位、规划和控制都会看起来像算法坏了。
...
画出 map、odom、base_link、laser、camera 的 TF 树，说明每条变换谁发布、频率多少、时间戳来自哪里。
...
- 能解释 REP 103/105、右手坐标系、map/odom/base_link、静态外参、动态 TF 和时间同步。
...
坐标章节要优先核对 REP 103/105、TF 树连通性、静态外参和时间戳。大量机器人问题表现为算法异常，根因却是坐标轴方向、frame_id 或 TF 时间不一致。
