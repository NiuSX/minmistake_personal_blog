...
这一章讲的是 **04. 机械臂运动学**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。
...
## 2026-06 深化精讲补充：机械臂运动学

Last researched: 2026-06-16

### 为什么这部分是机器人学习主干

机械臂运动学解决的是关节空间和任务空间之间的映射。能不能到达、以什么姿态到达、靠近目标时是否稳定，都取决于这个映射是否被正确建模。 机器人学习最容易出现的误区，是把理论看成考试内容，把 ROS、Gazebo、Nav2、MoveIt 2 看成工程工具。实际项目里二者不能分开：理论负责定义正确性，工程负责让正确性在有噪声、有延迟、有版本差异的系统中落地。机械臂运动学 的学习目标不是“能看懂一页公式”，而是能在系统出问题时，把现象还原成状态、模型、坐标、时间、约束和误差。

一个可执行的学习判断标准是：看到一条 ROS 消息，你能说出它的物理意义、单位、坐标系、时间戳、噪声来源和下游消费者；看到一个算法结果，你能判断它违反了哪个假设；看到一个仿真异常，你能判断是模型、控制、传感器还是任务逻辑的问题。

### 核心心智模型

机械臂可以看成一串刚体变换链：每个关节贡献一个局部变换，串联后得到末端位姿；反向问题则是在约束和多解之间选择一个可执行解。 这个模型可以贯穿移动机器人、机械臂、无人机和仿真系统。状态可以是二维位姿、三维位姿、关节角、速度、地图、物体位姿或任务状态；模型可以是运动学、动力学、传感器投影、碰撞约束或行为树转移；误差可以来自定位、控制、观测、约束违反或任务失败。

```mermaid
flowchart LR
  A[问题定义: 机械臂运动学] --> B[状态/模型]
  B --> C[坐标系与时间]
  C --> D[算法或控制器]
  D --> E[ROS 2 接口]
  E --> F[仿真或实机验证]
...
从关节角算末端位姿，适合显示、碰撞检测、状态估计和轨迹验证。URDF/TF 里的 link-joint 链就是工程版正运动学模型。

在学习 `机械臂运动学` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 2. 逆运动学

从目标位姿求关节角，可能无解、多解或数值不稳定。实际系统还要考虑关节限位、碰撞、姿态约束和上一次解的连续性。

在学习 `机械臂运动学` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 3. 雅可比

把关节速度映射到末端线速度和角速度，也用于力矩映射和奇异性分析。接近奇异位形时，小的末端速度可能需要极大的关节速度。

在学习 `机械臂运动学` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 4. MoveIt 2 关系

MoveIt 2 依赖 URDF/SRDF、运动学插件、规划场景和控制器接口。建模错误通常比算法选择更早造成规划失败。

在学习 `机械臂运动学` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 最小工程例子

一个 6 自由度机械臂末端要保持水平抓取杯子。IK 不仅要满足末端位置，还要满足姿态约束和避障约束。若只看位置误差，规划器可能给出末端翻转的解；若关节限位没有写进 URDF，仿真中可达的姿态实机可能撞到机械限位。
...
如果要在项目或面试中说明自己理解 机械臂运动学，不要只说“学过某算法”。更好的表达是：我知道这个模块解决什么问题，输入输出是什么，关键假设是什么，在哪些情况下会失败，失败时如何定位，和 ROS 2 中哪些话题、TF、参数或控制器相关。这样的表达比堆算法名更接近真实机器人研发。
...
机械臂运动学 的学习不应停留在名词解释。它最终要解决的是：当机器人系统出现不符合预期的运动、观测、规划或任务行为时，开发者能把现象拆成可验证的问题，并能用数据证明修复是否有效。围绕本主题，核心关注点是：理解关节空间和任务空间之间的映射，掌握正逆运动学、雅可比、奇异位形和 TCP 建模。
...
最小例子不是玩具，而是工程验证的基准。对于 机械臂运动学，可以按以下顺序构造：
