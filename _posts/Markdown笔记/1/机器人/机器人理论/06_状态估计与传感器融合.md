...
这一章讲的是 **06. 状态估计与传感器融合**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

状态估计是在噪声和漂移中回答“机器人现在在哪、速度多少、信不信这个结果”。
...
用轮速里程计和 IMU 做一个二维融合例子，比较只用 odom 与加入 IMU 后的轨迹和协方差变化。
...
- 能解释里程计、IMU、协方差、EKF/UKF、预测更新、传感器外参、时间同步和不确定性。
...
## 2026-06 深化精讲补充：状态估计与传感器融合

Last researched: 2026-06-16

### 为什么这部分是机器人学习主干

机器人做决策前必须先知道自己在哪里、速度多少、姿态如何以及这些估计有多可靠。状态估计的任务不是找一个看起来平滑的轨迹，而是给出可用于控制和规划的可信状态。 机器人学习最容易出现的误区，是把理论看成考试内容，把 ROS、Gazebo、Nav2、MoveIt 2 看成工程工具。实际项目里二者不能分开：理论负责定义正确性，工程负责让正确性在有噪声、有延迟、有版本差异的系统中落地。状态估计与传感器融合 的学习目标不是“能看懂一页公式”，而是能在系统出问题时，把现象还原成状态、模型、坐标、时间、约束和误差。

一个可执行的学习判断标准是：看到一条 ROS 消息，你能说出它的物理意义、单位、坐标系、时间戳、噪声来源和下游消费者；看到一个算法结果，你能判断它违反了哪个假设；看到一个仿真异常，你能判断是模型、控制、传感器还是任务逻辑的问题。

### 核心心智模型

预测来自运动模型，修正来自观测模型，协方差描述信任程度，异常值处理防止坏观测毁掉整个状态。 这个模型可以贯穿移动机器人、机械臂、无人机和仿真系统。状态可以是二维位姿、三维位姿、关节角、速度、地图、物体位姿或任务状态；模型可以是运动学、动力学、传感器投影、碰撞约束或行为树转移；误差可以来自定位、控制、观测、约束违反或任务失败。

```mermaid
flowchart LR
  A[问题定义: 状态估计与传感器融合] --> B[状态/模型]
  B --> C[坐标系与时间]
  C --> D[算法或控制器]
  D --> E[ROS 2 接口]
  E --> F[仿真或实机验证]
...
用先验、运动模型和观测更新逐步形成后验。它给出了 KF、EKF、粒子滤波等方法的统一视角。

在学习 `状态估计与传感器融合` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 2. EKF/UKF

EKF 用雅可比线性化非线性模型，UKF 用采样点传播均值和协方差。选择时要看非线性强度、计算资源和调参可控性。

在学习 `状态估计与传感器融合` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 3. 传感器角色

轮速短期平滑但会打滑，IMU 高频但会漂移，视觉/雷达定位可校正漂移但容易受环境影响，GNSS 适合室外但会跳变。

在学习 `状态估计与传感器融合` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 4. 工程融合

实际融合必须统一时间戳、坐标系、协方差、频率和外参。任何一个错了，滤波器都可能比单传感器更差。

在学习 `状态估计与传感器融合` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 最小工程例子

室内移动机器人常用轮速里程计 + IMU 融合发布 `odom -> base_link`，AMCL 或 SLAM 再发布 `map -> odom`。如果 IMU yaw 方向和 REP 103 不一致，EKF 输出会在转弯时逐渐偏离，Nav2 表现为路径跟踪越来越差。
...
如果要在项目或面试中说明自己理解 状态估计与传感器融合，不要只说“学过某算法”。更好的表达是：我知道这个模块解决什么问题，输入输出是什么，关键假设是什么，在哪些情况下会失败，失败时如何定位，和 ROS 2 中哪些话题、TF、参数或控制器相关。这样的表达比堆算法名更接近真实机器人研发。
...
状态估计与传感器融合 的学习不应停留在名词解释。它最终要解决的是：当机器人系统出现不符合预期的运动、观测、规划或任务行为时，开发者能把现象拆成可验证的问题，并能用数据证明修复是否有效。围绕本主题，核心关注点是：用预测、观测、协方差和异常值处理把多传感器数据融合成可用于规划和控制的状态。
...
最小例子不是玩具，而是工程验证的基准。对于 状态估计与传感器融合，可以按以下顺序构造：
