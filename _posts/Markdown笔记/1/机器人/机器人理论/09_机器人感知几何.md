...
这一章讲的是 **09. 机器人感知几何**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

机器人感知要把传感器数据变成可用于定位、避障、抓取或决策的几何和语义信息。
...
选择激光雷达或相机数据，标出坐标系、时间戳、外参、噪声、滤波和最终输出的障碍物或目标位姿。
...
- 能解释相机模型、点云、激光扫描、外参标定、几何约束、噪声和感知结果如何进入规划控制。
...
## 2026-06 深化精讲补充：机器人感知几何

Last researched: 2026-06-16

### 为什么这部分是机器人学习主干

感知几何把像素、深度、点云和机器人坐标系连起来。没有几何，检测框只是图片上的矩形，不能变成机器人能抓取或避障的空间信息。 机器人学习最容易出现的误区，是把理论看成考试内容，把 ROS、Gazebo、Nav2、MoveIt 2 看成工程工具。实际项目里二者不能分开：理论负责定义正确性，工程负责让正确性在有噪声、有延迟、有版本差异的系统中落地。机器人感知几何 的学习目标不是“能看懂一页公式”，而是能在系统出问题时，把现象还原成状态、模型、坐标、时间、约束和误差。

一个可执行的学习判断标准是：看到一条 ROS 消息，你能说出它的物理意义、单位、坐标系、时间戳、噪声来源和下游消费者；看到一个算法结果，你能判断它违反了哪个假设；看到一个仿真异常，你能判断是模型、控制、传感器还是任务逻辑的问题。

### 核心心智模型

从 3D 世界点到 2D 像素是投影，从像素和深度回到 3D 是反投影，多传感器融合依赖外参和时间同步。 这个模型可以贯穿移动机器人、机械臂、无人机和仿真系统。状态可以是二维位姿、三维位姿、关节角、速度、地图、物体位姿或任务状态；模型可以是运动学、动力学、传感器投影、碰撞约束或行为树转移；误差可以来自定位、控制、观测、约束违反或任务失败。

```mermaid
flowchart LR
  A[问题定义: 机器人感知几何] --> B[状态/模型]
  B --> C[坐标系与时间]
  C --> D[算法或控制器]
  D --> E[ROS 2 接口]
  E --> F[仿真或实机验证]
...
内参描述焦距和主点，畸变参数描述镜头偏差。标定质量直接影响深度估计、PnP 和点云配准。

在学习 `机器人感知几何` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 2. 外参

外参描述相机相对机器人或其他传感器的位姿。外参错一点，远距离投影会偏很多。

在学习 `机器人感知几何` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 3. 深度与点云

深度相机提供每个像素的距离，可反投影为点云。点云还需要滤波、下采样、坐标变换和地面分割。

在学习 `机器人感知几何` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 4. 视觉与机器人

目标检测要变成抓取点，需要相机内参、深度、相机到机械臂或底盘的 TF，以及任务坐标系下的可达性判断。

在学习 `机器人感知几何` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 最小工程例子

相机识别到杯子中心像素 `(u,v)`，深度为 `z`。通过内参反投影得到 `p_camera`，再用 `T_base_camera` 变换到机械臂 base 坐标系，最后 IK 才能求抓取姿态。任何一步 frame 或时间错了，机械臂都会抓偏。
...
- 只保存图像不保存 CameraInfo
- 相机 optical frame 方向没建
- 深度和彩色图未对齐
- 点云 frame_id 和 TF 不连通
- 手眼标定后没有验证多个空间点误差
...
如果要在项目或面试中说明自己理解 机器人感知几何，不要只说“学过某算法”。更好的表达是：我知道这个模块解决什么问题，输入输出是什么，关键假设是什么，在哪些情况下会失败，失败时如何定位，和 ROS 2 中哪些话题、TF、参数或控制器相关。这样的表达比堆算法名更接近真实机器人研发。
...
机器人感知几何 的学习不应停留在名词解释。它最终要解决的是：当机器人系统出现不符合预期的运动、观测、规划或任务行为时，开发者能把现象拆成可验证的问题，并能用数据证明修复是否有效。围绕本主题，核心关注点是：把像素、深度、点云、外参和机器人坐标系连接起来，让感知结果能被抓取、避障和定位使用。
...
最小例子不是玩具，而是工程验证的基准。对于 机器人感知几何，可以按以下顺序构造：
