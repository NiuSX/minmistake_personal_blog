...
这一章讲的是 **11. 任务规划、状态机与行为树**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

任务规划把底层动作组织成可恢复的行为流程，让机器人知道下一步做什么、失败后怎么处理。
...
为“去目标点、识别物体、抓取、返回”画一个状态机或行为树，并标出每个动作的成功、失败和超时分支。
...
- 能解释状态机、行为树、Action、恢复行为、超时、重试和任务级安全边界。
...
- [MoveIt 2 Documentation](https://moveit.picknik.ai/main/index.html)
## 2026-06 深化精讲补充：任务规划、状态机与行为树

Last researched: 2026-06-16

### 为什么这部分是机器人学习主干

机器人不是只执行单个算法，而是在感知、导航、操作、异常恢复和人机交互之间不断切换。任务层决定系统在复杂场景中是否可理解、可恢复、可维护。 机器人学习最容易出现的误区，是把理论看成考试内容，把 ROS、Gazebo、Nav2、MoveIt 2 看成工程工具。实际项目里二者不能分开：理论负责定义正确性，工程负责让正确性在有噪声、有延迟、有版本差异的系统中落地。任务规划、状态机与行为树 的学习目标不是“能看懂一页公式”，而是能在系统出问题时，把现象还原成状态、模型、坐标、时间、约束和误差。

一个可执行的学习判断标准是：看到一条 ROS 消息，你能说出它的物理意义、单位、坐标系、时间戳、噪声来源和下游消费者；看到一个算法结果，你能判断它违反了哪个假设；看到一个仿真异常，你能判断是模型、控制、传感器还是任务逻辑的问题。

### 核心心智模型

状态机适合状态少且转移清晰的流程，行为树适合层级任务、条件判断、重试和恢复动作组合。 这个模型可以贯穿移动机器人、机械臂、无人机和仿真系统。状态可以是二维位姿、三维位姿、关节角、速度、地图、物体位姿或任务状态；模型可以是运动学、动力学、传感器投影、碰撞约束或行为树转移；误差可以来自定位、控制、观测、约束违反或任务失败。

```mermaid
flowchart LR
  A[问题定义: 任务规划、状态机与行为树] --> B[状态/模型]
  B --> C[坐标系与时间]
  C --> D[算法或控制器]
  D --> E[ROS 2 接口]
  E --> F[仿真或实机验证]
...
状态和转移显式，适合开机、自检、待命、执行、故障等生命周期流程。状态爆炸是主要问题。

在学习 `任务规划、状态机与行为树` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 2. 行为树

用 Sequence、Fallback、Condition、Action 等节点组合任务。Nav2 使用行为树协调规划、控制、恢复和目标检查。

在学习 `任务规划、状态机与行为树` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 3. 恢复策略

机器人任务必须考虑失败：定位丢失、路径被堵、控制超时、电量不足、传感器异常。恢复动作应有边界，不能无限重试。

在学习 `任务规划、状态机与行为树` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 4. 系统集成

任务层通过 Action 调用导航或机械臂，通过 Service 做短操作，通过 Topic 监听状态，通过参数配置策略。

在学习 `任务规划、状态机与行为树` 时，这一模块要同时从公式和工程接口两侧理解：公式告诉你变量之间的关系，工程接口告诉你这些变量来自哪个话题、哪个坐标系、哪个配置文件或哪个传感器。只会公式但不知道数据来源，会在真实系统中无法排错；只会命令但不知道模型假设，会在参数失效时找不到根因。

### 最小工程例子

巡检机器人任务可以写成：检查电量 -> 导航到点位 -> 拍照 -> 上传 -> 下一个点位。若导航失败，先清理 costmap，再重新规划，再尝试后退，仍失败则上报人工。行为树能把这些恢复策略组织在局部子树里，而不是散落在回调函数中。
...
如果要在项目或面试中说明自己理解 任务规划、状态机与行为树，不要只说“学过某算法”。更好的表达是：我知道这个模块解决什么问题，输入输出是什么，关键假设是什么，在哪些情况下会失败，失败时如何定位，和 ROS 2 中哪些话题、TF、参数或控制器相关。这样的表达比堆算法名更接近真实机器人研发。
...
任务规划、状态机与行为树 的学习不应停留在名词解释。它最终要解决的是：当机器人系统出现不符合预期的运动、观测、规划或任务行为时，开发者能把现象拆成可验证的问题，并能用数据证明修复是否有效。围绕本主题，核心关注点是：把导航、操作、检测、恢复和人机交互组织成可解释、可取消、可恢复的任务流程。
...
最小例子不是玩具，而是工程验证的基准。对于 任务规划、状态机与行为树，可以按以下顺序构造：
