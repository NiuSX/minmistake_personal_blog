...
这一章讲的是 **12. 机器人安全、可靠性与系统工程**。阅读时不要只记命令、参数或算法名，而要把它放进机器人闭环：模型是否描述真实机器人，坐标系和时间是否一致，传感器数据是否可信，状态估计是否稳定，规划结果是否可执行，控制命令是否安全，仿真和真机差异是否被验证。机器人学习的关键不是让 demo 偶然跑通，而是能解释每个模块为什么工作、怎样失败、如何调试。

### 一句话先懂

机器人安全不是最后加急停按钮，而是从硬件、控制、软件、任务、测试和运维各层限制危险动作。
...
为一个移动机器人写安全清单：速度限制、急停、碰撞监测、失联停车、低电量策略、传感器故障和人工接管。
...
- 能解释失效模式、冗余、急停、限速、看门狗、故障恢复、测试验收和现场运维安全。
...
安全章节要把风险写进系统工程：急停、限速、碰撞监测、通信失联、低电量、传感器故障、人工接管和测试验收都要有可验证机制。
